...
export interface InputProps {
  ...
  className?: string;
  type?: string;
  verificate?: (value: string) => string | null; // ✅ nouvelle prop
}
...
const Input = ({
  ...
  type = "text",
  onChange,
  verificate, // ✅ récupération de la prop
}: InputProps) => {
  const [localValue, setLocalValue] = useState(value);
  const [showCalendar, setShowCalendar] = useState(false);
  const [error, setError] = useState<string | null>(null); // ✅ erreur locale
  ...
  return (
    <div className={`wrapper ${className}`}>
      ...
      {error && <div className="text-red-500 text-sm mt-1">{error}</div>} {/* ✅ erreur affichée ici */}
